...
def replaceEntitiesWithTags(sentence, entity1_start, entity1_end, entity2_start, entity2_end):
    return sentence[:entity1_start] + 'PROTEIN1' + sentence[entity1_end:entity2_start] + 'PROTEIN2' + sentence[entity2_end:]
...
def generate_entity_pairs(processed_paper, sentences):
    ...
    for sentence, entities in entities_in_sentences.items():
        for entity1, entity2 in combinations(entities, 2):
            ...
            if len(entity1[4]) > 0 and len(entity2[4]) > 0:
                entity_pairs.append((id_, sentence, new_sentence, entity1[2], entity1[4][0], entity2[2], entity2[4][0]))
...
def get_all_entity_pairs_with_file(input_path):
    papers = process_file(input_path)
...
def get_all_entity_pairs_with_string(content):
    papers = content.strip().split("\n\n")
    processed_papers = process_papers(papers)
...
def main():
    ...
    entity_pairs_all = get_all_entity_pairs_with_file(args.input)
